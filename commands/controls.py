...
def register_controls(client: Client, relay_manager: RelayManager):
    """Register Pyrogram control command handlers."""

    @client.on_message(filters.command("startrelay") & (filters.me | filters.private | filters.group))
    ...
    @client.on_message(filters.command("stoprelay") & (filters.me | filters.private | filters.group))
    ...
    @client.on_message(filters.command("status") & (filters.me | filters.private | filters.group))
    async def status_cmd(c: Client, m: Message):
        """Handler for /status"""
        status_text = relay_manager.get_status()
        await m.reply_text(status_text)

    @client.on_message(filters.command("recstats") & (filters.me | filters.private | filters.group))
    ...
    @client.on_message(filters.command("setdelay") & (filters.me | filters.private | filters.group))
    ...
    logger.info("Control command handlers registered.")
